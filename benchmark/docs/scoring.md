...
| H11-remote-result-boundary-trap | DSH-0.1.2-A2-02 `RemoteResult` shape; API-02 `RemoteResult` version boundaries; rollup "Remote call error flow" | Read-only, report-only, six canonical sections graded independently on their own text: Root Cause 20 (paired: ordinary failure → ok:false AND does not reject / never enters catch), Problems 10 (≥2 real wrongness items), Corrected Implementation 25 (fenced code only: awaits the remote call 5 + explicit failure branch 8 + accesses result.error 6 + result.value only after the branch 6), RemoteResult Control Flow 20 (ok:true/value 5 + ok:false/error 5 + Promise stays resolved 5 + catch is not the business-failure path 5), Reject Boundary 15 (assembly/programming faults can reject 5 + concrete example 5 + genuine rejects propagate 5), Error Boundary 10 (no instanceof / branch on code 5 + isRemoteFailure only at a true throw/catch boundary 5). Hard caps: throw-centric claim ("ordinary failures throw / handle in catch") → 30; report never mentions result.ok → 50; fenced fix reads result.value without a prior ok branch → 60; fenced fix discriminates with instanceof RemoteError → 60; fenced catch swallows/retries rejects → 60; fixture modified → 0. This task deliberately does NOT grade error-code migration or retry policy as its primary axis: H6-remote-error-trap owns error vocabulary/retry behavior, while H11 grades the RemoteResult resolved-vs-rejected control-flow boundary |
