# H11-remote-result-boundary-trap · RemoteResult control-flow boundary (Read-Only Markdown Diagnosis)
...
unknown handling, silent-swallow avoidance). H11 deliberately starts after that
...
- **Oracle**: `harbor run -p benchmark/tasks/H11-remote-result-boundary-trap -a oracle`, expected reward 1.0.
...
| C · H6-perfect but H11-wrong (differential) | a thorough H6-style answer (code vocabulary, retry policy, no instanceof) that still claims failures throw / are handled in catch and never mentions the ok:false / no-reject boundary | ≤ 0.30 |
