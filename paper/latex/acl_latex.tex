...
% We anonymize the name of the framework studied in this paper.
% Replace \framework with the real name upon de-anonymization.
\newcommand{\framework}{\textsc{Heron}}
...
Cards, rather than raw issues, are the validated ground truth from which both the skill corpus and the benchmark tasks are derived (\S\ref{sec:construction}); Table~\ref{tab:card} shows one card (anonymized).

\begin{table}[t]
\centering
\fbox{\parbox{0.92\columnwidth}{\centering\vspace{1.6cm}
Placeholder: one upgrade card instance (symptom / root cause / fix / affected versions / provenance link), anonymized.
\vspace{1.6cm}}}
